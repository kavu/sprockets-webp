    ...
    class Converter
      class << self
        ...
        def webp_file_by_config(config, data)
          ...
          logger.info '********************************************************************************'
          logger.info "context.metadata: #{context.metadata[:dependencies].to_a.last}"
          logger.info "context: #{context.load(context.metadata[:dependencies].to_a.last.gsub('file-digest', 'file'))}"
